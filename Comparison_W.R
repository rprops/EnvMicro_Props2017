...
div.16S <- read.csv2("otu.diversity16S_W.csv")
...
metadata$Sample_16S <- gsub(metadata$Sample_16S, pattern="F", replace="W")
metadata$Sample_16S[grep(metadata$Sample_16S , pattern="*.W.*")] <- paste(metadata$Sample_16S[grep(metadata$Sample_16S , pattern="*.W.*")],"D",sep="")
...
### Explorative plot
png("alpha-div_WHOLE_log.png",width=7*1.65,height=5*1.5,res=500,units="in")
...
### Get R squared
lm.W <- lm(log2(D2)~log2(D2.fcm), data=data.total.final)
summary(lm.W)$r.squared
